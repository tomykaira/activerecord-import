...
def should_support_postgresql_upsert_functionality
  describe "#import" do
    context "with :on_duplicate_key_update option" do
      ...
      context "given columns and values with :validation checks turned off" do
        ...
        macro(:perform_import) do |*opts|
          Topic.import columns, updated_values, opts.extract_options!.merge(:on_duplicate_key_update => { key: :id, columns: update_columns }, :validate => false)
        end
        ...
        context "using an empty value for column names" do
          let(:update_columns){ "" }
      ...
      context "given array of model instances with :validation checks turned off" do
        macro(:perform_import) do |*opts|
          ...
          @topic.parent_id = 57
          Topic.import [@topic], opts.extract_options!.merge(:on_duplicate_key_update => { key: :id, columns: update_columns }, :validate => false)
        end
      ...
      context "without specifying a unique constraint" do
        let(:columns){  %w( id title author_name author_email_address parent_id ) }
        ...
        macro(:perform_import) do |*opts|
          Topic.import columns, updated_values, opts.extract_options!.merge(:on_duplicate_key_update => { columns: [:title] }, :validate => false)
        end
      ...
      context "without specifying update columns" do
        let(:columns){  %w( id title author_name author_email_address parent_id ) }
        let(:values){ [ [ 100, "Book", "John Doe", "john@doe.com", 17 ] ] }
        let(:updated_values){ [ [ 100, "Book - 2nd Edition", "This should raise an exception", "john@nogo.com", 57 ] ] }

        macro(:perform_import) do |*opts|
          Topic.import columns, updated_values, opts.extract_options!.merge(:on_duplicate_key_update => { key: :id }, :validate => false)
        end
